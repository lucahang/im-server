...
class MsgManager {
public:
    explicit MsgManager(RedisClient& redis);
...
private:
    int64_t NextMsgId();
    std::string MakeSingleSessionId(const std::string& uid1, const std::string& uid2);
    std::string MakeGroupSessionId(const std::string& group_id);
