...
MsgManager::MsgManager(RedisClient& redis, Database& db) 
        : redis_(redis),db_(db) {}
...
void MsgManager::SaveMessage(const im::HistoryMessage& msg) {
    redis_.SaveMessage(msg);
    db_.SaveMessage(msg,MakeSingleSessionId(msg.sender(), msg.receiver()));
}
...
std::vector<im::HistoryMessage> MsgManager::GetHistory(const std::string& uid,
    ...
    // 2. Redis 未命中，从 MySQL 回源
    std::string session_id = is_group ? MakeGroupSessionId(peer_id) : MakeSingleSessionId(uid, peer_id);
    auto db_msgs = db_.GetMessagesBySession(session_id, start, count);

    // 可选：将结果异步回填到 Redis（简单起见这里同步写入，实际可投递到线程池）
    for (const auto& msg : db_msgs) {
        redis_.SaveMessage(msg);   // 自定义辅助函数，重建 Redis 中的数据
    }
    return db_msgs;
}
